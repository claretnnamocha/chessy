class Players {
    ...
    add(number, id, info, max_no, no_of_pins, base, pin) {
        ...
        if (Object.keys(this.players).length < max_no) {
            this.players[number] = { id: id, number: number, info: info, game: { pin: pin, valid: Positive, points: info.game.points, saveable_point: 0 } }
            ...
            //publish player
            console.log("creating player ", this.GeneratorObject.get_player(), id, number)
            if(this.GeneratorObject.get_player() == number) this.GeneratorObject.publish({number: number, id: id, info: info, max_no: max_no, no_of_pins: no_of_pins, base: base, pin: pin}, publish_action.player_create, publish_source.player)
